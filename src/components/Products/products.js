...
// Move ProductModal outside of Products component
const ProductModal = ({ isEdit = false, show, onClose, onSubmit, formData, setFormData, loading }) => {
  if (!show) return null;
...
const Products = ({ products, setProducts, setScanMode, fetchProducts }) => {
  ...
  const getStockStatus = (stock, minStock) => {
    if (stock <= minStock) return { status: 'low', color: 'text-red-400', icon: AlertTriangle };
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {filteredProducts.map((product) => {
          ...
          return (
            <div key={product.id} className="bg-gray-800 border border-gray-700 rounded-xl p-6 hover:border-gray-600 transition-colors">
              {product.image_url && (
                <img
